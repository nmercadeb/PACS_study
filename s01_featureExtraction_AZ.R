...
source("./s01_forFeatureExtraction_AZ.R")
...
save(list = c("discrete_table", "continuous_table", "factor_table"), 
     file = "RData/s01_FE_prep_AZ.Rdata")
...
save(list = c("lasso_data","var.lasso_reg"), file = "RData/s01_FE_lasso_data_AZ.Rdata")
save(list = c("lasso_reg"), file = "RData/s01_FE_lasso_regression_AZ.Rdata")

print("Get PROPENSITY SCORES for AZ")
## PROPENSITY SCORES ---------------------
...
save(list=c("model_base"), file = "RData/s01_FE_PS_model_base_AZ.Rdata")
save(list=c("compulsory_features","data_base","PS", "propensityScores"), file = "RData/s01_FE_PS_data_AZ.Rdata")
write.csv(propensityScores, file = "./Data/s01_ps_AZ.csv")
...
ggsave(width = 7, height = 4, dpi = 300,
       filename = "./BalancePlots/s01_PS_AZ.png")
...
save(list = c("balance_tab_before","continuous_smd_before","discrete_smd_before", "overall_factor_smd_before"), file = "RData/s01_FE_balance_before_AZ.Rdata")
write.csv(balance_tab_before,"./Data/s01_balance_table_before_AZ.csv")
...
save(list = c("balance_tab_after","continuous_smd_after","discrete_smd_after", "overall_factor_smd_after"), file = "RData/s01_FE_balance_after_AZ.Rdata")
...
ggsave(width = 7, height = 4, dpi = 300,
       filename = "./BalancePlots/s01_smd_AZ.png")
...
save(list = c("toPlot_NCO_before","data_NCO","toPlot_NCO_after"), file = "./RData/s01_FE_NCO_AZ.Rdata")
write.csv(toPlot_NCO_before,"./Data/s01_NCO_before_AZ.csv")
write.csv(toPlot_NCO_after,"./Data/s01_NCO_after_AZ.csv")

individuals_AZ <- individuals_cru %>% inner_join(propensityScores) %>%
  mutate(weight = ifelse(group == "vaccinated", 1-ps, ps)) %>% select(-ps)
write.csv(individuals_AZ %>% select(group, index_date), file = "./Data/s01_individuals_AZ.csv")
save("individuals_AZ", file = "./RData/s01_individuals_AZ.Rdata")
...
write.csv(toPlot_NCO_calibrated, file = "./Data/s01_NCO_calibrated_AZ.csv")
